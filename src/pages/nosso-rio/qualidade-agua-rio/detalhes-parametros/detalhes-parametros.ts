...
export class DetalhesParametros {
  ...
  carregarParametros(){
    ...
    if(this.parametro == 'DQO'){
      texto = `<div><p>${message}</p></div>`


      /*A Demanda Química de Oxigênio, identificada pela sigla DQO, é um parâmetro indispensável nos 
   ...
   /* else if(this.parametro == 'DBO'){
      texto = `A Demanda Bioquímica de Oxigênio representa a quantidade de oxigênio necessária para oxidar`;
          
    }
    else if(this.parametro == 'pH - Potencial Hidrogeniônico'){
      texto = `O pH afeta o metabolismo de várias espécies aquáticas. A Resolução `+
                `CONAMA 357 estabelece que para a proteção da vida aquática o pH deve estar entre 6 e 9.`+
                `<br>Alterações nos valores de pH também podem aumentar o efeito de substâncias químicas `+
                `que são tóxicas para os organismos aquáticos, tais como os metais pesados.`;
    }
     else if(this.parametro == 'Oxigênio Dissolvido'){
        texto = 'O oxigênio dissolvido é vital para a preservação da vida aquática, já que vários organismos '+
                ...
                'matéria orgânica.<br> Por outro lado as águas limpas apresentam concentrações de oxigênio '+
                'dissolvido mais elevadas, geralmente superiores a 5mg/L, exceto se houverem condições naturais '+
                'que causem baixos valores deste parâmetro. <br>As águas eutrofizadas (ricas em nutrientes) '+
                ...
                'mortandades de peixes.<br> Além da fotossíntese, o oxigênio também é introduzido nas águas '+
        ...
        texto = 'Nos corpos d’água o nitrogênio pode ocorrer nas formas de nitrogênio orgânico, amoniacal, '+
                'nitrito e nitrato. Os nitratos são tóxicos aos seres humanos, e em altas concentrações causa '+
                'uma doença chamada metahemoglobinemia infantil, que é letal para crianças.<br> Pelo fato '+
                ...
                '<br> As fontes de nitrogênio para os corpos d’água são variadas, sendo uma das principais o '+
                ...
                '<br> Também ocorre a fixação biológica do nitrogênio atmosférico pelas algas e bactérias. '+
          ...
          texto = 'Do mesmo modo que o nitrogênio, o fósforo é um importante nutriente para os processos biológicos '+
                'e seu excesso pode causar a eutrofização das águas. Para conhecer mais sobre eutrofização, veja o '+
                'item sobre o Índice de Estado Trófico.<br> Entre as fontes de fósforo destacam-se os esgotos '+
          ...
          texto =  'O resíduo total é a matéria que permanece após a evaporação, secagem ou calcinação da amostra '+
                'de água durante um determinado tempo e temperatura.<br> Quando os resíduos sólidos se depositam '+
        ...
        texto = 'A turbidez indica o grau de atenuação que um feixe de luz sofre ao atravessar a água. Esta '+
                'atenuação ocorre pela absorção e espalhamento da luz causada pelos sólidos em suspensão '+
                '(silte, areia, argila, algas, detritos, etc. ).<br> A principal fonte de turbidez é a erosão '+
                ...
                'turbidez das águas.<br> O aumento da turbidez faz com que uma quantidade maior de produtos '+
         ...
         texto = 'A condutividade elétrica da água representa a facilidade ou dificuldade de passagem da '+
         ...
         texto = 'As bactérias coliformes termotolerantes ocorrem no trato intestinal de animais de sangue quente '+
